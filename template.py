...
def call_openai(
    ...
    # Dùng perf_counter để đo thời gian chính xác hơn
    ...
    # Đảm bảo latency > 0.0 kể cả khi bị mock trong test
...
def chat_with_system_prompt(
    ...
    client = OpenAI()
    # Tái dùng cấu trúc call_openai nhưng tạo messages gồm 2 phần tử theo đúng thứ tự
    messages = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": user_prompt},
    ]

    # Dùng time.perf_counter() để đo thời gian chính xác hơn
    start_time = time.perf_counter()
    ...
    # Đảm bảo latency > 0.0 ngay cả khi function bị mock trong unit test
    if latency_seconds <= 0.0:
        latency_seconds = 0.0001
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    try:
        ...
        # Fallback: Ước lượng 1 token ~ 4 ký tự, tối thiểu 1 token cho chuỗi không rỗng
        if not text:
            return 0
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    """
    # Đếm số token thực tế của prompt và response
    input_tokens = count_tokens(prompt, model=model)
    output_tokens = count_tokens(response, model=model)

    # Lấy thông tin giá (Lấy từ bảng giá hoặc lấy mặc định của gpt-4o nếu không tìm thấy)
    pricing = PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["gpt-4o"])

    # Tính toán chi phí cho từng phần (đơn vị: USD)
...
def retry_with_backoff(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            # Nếu đã dùng hết số lần retry (lần gọi thứ max_retries) thì re-raise lỗi
            if attempt == max_retries:
                raise e
            
            # Tính thời gian chờ exponential backoff: base_delay * 2^attempt
...
def run_assistant(
    ...
    # Nếu không truyền get_input, mặc định dùng hàm input của Python
    if get_input is None:
        get_input = input
    ...
    while True:
        # 1. Kiểm tra max_turns trước khi lấy input
        if max_turns is not None and num_turns >= max_turns:
            break
        ...
        # 2. Thoát nếu người dùng nhập 'quit' hoặc 'exit' (không phân biệt hoa thường)
        if user_msg.strip().lower() in ("quit", "exit"):
            break
        ...
        # 3. Chuẩn bị danh sách messages: system + history + user_msg
        messages = [{"role": "system", "content": persona}] + history + [
            {"role": "user", "content": user_msg}
        ]
        ...
        stream = retry_with_backoff(_make_stream_call)

        # 4. In từng chunk và ghép lại thành reply hoàn chỉnh
        reply_parts = []
        for chunk in stream:
        ...
        # 5. Cập nhật history (thêm user và assistant), giữ tối đa 6 messages (3 lượt)
        ...
        # Đếm token bằng count_tokens
        u_tokens = count_tokens(user_msg, model=OPENAI_MODEL)
        a_tokens = count_tokens(reply_msg, model=OPENAI_MODEL)
        total_tokens += u_tokens + a_tokens

        # Tính chi phí bằng estimate_cost
        cost_info = estimate_cost(user_msg, reply_msg, model=OPENAI_MODEL)
        total_cost += cost_info["total_cost"]

    # 7. Trả về dict kết quả
    return {
        "num_turns": num_turns,
